...
import getAccountCreationPage from "@salesforce/apex/NewAccountFormCls.getAccountCreationPage";
import createAccount from "@salesforce/apex/NewAccountFormCls.createAccount";
import deleteContentDocument from "@salesforce/apex/NewAccountFormCls.deleteContentDocument";
import relatedContentDocument from "@salesforce/apex/NewAccountFormCls.relatedContentDocument";
import getProgramList from "@salesforce/apex/NewAccountFormCls.getCurrentUserProgram";
import getEnrollmentRecordType from "@salesforce/apex/NewAccountFormCls.getEnrollmentRecordType";
import getServiceCreation from "@salesforce/apex/NewAccountFormCls.getServiceCreation";
import validateNpiStatus from "@salesforce/apex/NewAccountFormCls.validateNpiStatus";
import checkNPIDuplicate from "@salesforce/apex/NewAccountFormCls.checkNPIDuplicate";
import createEnrollmentService from "@salesforce/apex/NewAccountFormCls.createEnrollmentService";
import updateCase from "@salesforce/apex/NewAccountFormCls.updateCase";
import fetchRecordTypeId from "@salesforce/apex/NewAccountFormCls.fetchRecordTypeId";
import getCaseFieldAndValueMap from "@salesforce/apex/NewAccountFormCls.getCaseFieldAndValueMap";
import generateOutboutCallEvent from "@salesforce/apex/NewAccountFormCls.sendOutboundCommunication";
import duplicateEmailCheck from "@salesforce/apex/NewAccountFormCls.validateDuplicateEmail";
import calculateAgeInYears from "@salesforce/apex/REMSUtility.validateAge";
...
import getServiceConfigRecords from "@salesforce/apex/NewAccountFormCls.getServiceConfigRecords";
import getNameSpace from "@salesforce/apex/NewAccountFormCls.getNameSpace";
import getPermissionSets from "@salesforce/apex/NewAccountFormCls.getPermissionSets";
import getAccountfieldMapping from "@salesforce/apex/REMSAccountCreation.getAccountfieldMapping";
import checkDuplicate from "@salesforce/apex/REMSAccountCreation.checkDuplicate";
import createSubStakeHolderAccounts from "@salesforce/apex/REMSAccountCreation.createSubStakeHolderAccounts";
import getAssessmentResponse from "@salesforce/apex/NewAccountFormCls.getAssessmentResponse";
import primaryARDetails from "@salesforce/apex/NewAccountFormCls.getPrimaryARDetails";
...
import updateSubstakeholderData from "@salesforce/apex/NewAccountFormCls.updateSubstakeholder";
import getCasesForStakeholder from "@salesforce/apex/NewAccountFormCls.getCasesForStakeholder";
...
export default class StakeHolderAccountCreationCmp extends NavigationMixin(LightningElement) {
  @api selectedValue;
  @api recordTypeName;
  @api participantType;
  @api callingFrom;
...
@api showPopup = false;
 ...
 handleGetPermissionSets(){
  ...
  preFillCaseLayout = async (event) => {
    let nameSpace = this.namespaceVar;
    var fldNameKey4 = 'Phone';
    var fldNameKey5 = `${nameSpace}` + 'Phone__c';
    let Phone = this.template.querySelector(
      `lightning-input-field[data-field="${fldNameKey4}"]`
    );
    let PhoneCustom = this.template.querySelector(
      `lightning-input-field[data-field="${fldNameKey5}"]`
    );
    if (Phone != null) {
      Phone.value = this.phoneValue;
    }
    if (PhoneCustom != null) {
      PhoneCustom.value = this.phoneValue;
    }
      ...
      try {
        ...
        for (const key in this.orginalData) {
          this.orginalData[key].forEach((element) => {
            if(!(element.dependentField && this.skipFieldVisibility && !response.hasOwnProperty(element.dependentField))){
        ...
        for (const key in this.orginalData) {
          this.orginalData[key].forEach((element) => {
            ...
            if (hasNPIOverridePermission && element.fieldName === `${nameSpace}NPI_Status__c`) {
             element.editable = false;
            }
            if(this.userPermissionSets.includes(element.permissionSet)){
             element.editable = false;
            }
                      });
        }
    ...
    getAccountCreationPage({
      ...
      .then((result) => {
        ...
        if (result) {
          ...
          if (!currentUrl.includes("DocumentReview")) {
            if (
              this.serviceFields[0].value[1].fieldName ===`${nameSpace}Channel__c`
            ) {
              this.serviceFields[0].value[1].fieldValue = "Phone";
            }
                     }
                   this.fieldValues = arrayMapKeys.reduce(
            (acc, field) => [...acc, ...field.value],
          ...
          this.handleGetEnrollmentRecordType();
                  }
        this.ShowSpinner = false;
      })
      .catch((error) => {
  ...
  generateDupColumns(configs){
    ...
    OpenModelPopForDuplicateAccounts(data){
      ...
      this.isOpen = this.showFacility || this.showOffice || this.showPrescriber || this.showHCS || this.showMainstakeDupModal;
      if(!this.isOpen){
  ...
  async handleSubmit(event) {
    ...
    if(fields['Status'] === 'Complete'){
      ...
      if(this.serviceConfigRecord[`${nameSpace}Service_Record_Type__c`] == 'Change_Authorized_Representative'){
        ...
        if(blockedStatus.includes(fields['Status'])){
          this.ShowSpinner = false;
    ...
    if(!this.validFieldValues && this.isInputValid()){
      ...
      if(this.serviceConfigRecord){
        ...
        .then(async (result) => {
          ...
          if(this.serviceConfigRecord[`${nameSpace}Service_Record_Type__c`] == 'Change_Authorized_Representative' ){
            fields[`${nameSpace}Participant__c`] = fields[`${nameSpace}Healthcare_Setting__c`] || fields[`${nameSpace}Pharmacy__c`] ;
            this.recordTypeData = this.enrollmentRecordType;
  ...
  async handleOnSuccess(event) {
    ...
    if(this.serviceConfigRecord[`${nameSpace}Service_Record_Type__c`] == 'Change_Authorized_Representative' && this.isInputValid()){
      ...
      let accId = this.fields[`${nameSpace}Healthcare_Setting__c`] || this.fields[`${nameSpace}Pharmacy__c`];
      this.showToastNotification(
        "Success",
      ...
      if (response.id) {
        ...
        if( Object.keys(this.substakeHolderUpdateList).length>0){
          await updateSubstakeholderData({
            ...
            console.log(' StakeHolderAccountCreationCmp. errors: ', errors);
            this.ShowSpinner = false;
          });
        }
        if (this.serviceType && this.participantType && this.accountId) {
          ...
          await this.handleCreateEnrollmentService();
          await this.closeQuickSubtab(result);
          this.ShowSpinner = false;
        } else if (
          this.serviceType &&
  ...
  async validateDuplicateEmail(emailObj,parentFld,affPrimaryprtcpnt,parentEmailMap){
    ...
    for(var emailField in duplicateEmailList){
      ...
      if(emailErrFld.value){
        emailErrFld.value = emailErrFld.value+'Duplicate data found in '+errorMap;
      }else{  
        emailErrFld.value = 'Duplicate data found in '+errorMap;
      }
    }
  ...
  removeDuplicateErrorMessage(duplicateEmailList){
    for(var dupFieldName in duplicateEmailList){
      ...
      if(emailErrFld.value.includes('Duplicate data found')){
        emailErrFld.value = '';
  ...
  async handleInputChange(event) {
    ...
    const fieldValue = event.target.value;
    var ErrorMessage;
    if(this.agecalFields.includes(fieldName)){
      let ageDiff = await calculateAgeInYears({DOB:fieldValue});
      if(!fieldValue){
        ageDiff = false;
      }
      await this.handleAgeFields(ageDiff);
    }
...
if(this.serviceConfigRecord[`${nameSpace}Service_Record_Type__c`] == 'Change_Authorized_Representative' && (parentName == `${nameSpace}Healthcare_Setting__c` || parentName == `${nameSpace}Pharmacy__c`) && item.fieldName == `Status`){
  ...
  handleFieldDisplay(fieldVisible) {
    ...
    for (const key in this.orginalData) {
      this.orginalData[key].forEach((element) => {
        if (element.fieldName === npiOverrideField) {
              ...
              async (focusedTab) => {
                if(result.startsWith('001')){
                  ...
                  openTab({ recordId: result, focus: true })
                    .then((_) => {
                      if(this.serviceConfigRecord && Object.keys(this.serviceConfigRecord).length !== 0 && (this.serviceConfigRecord[`${this.namespaceVar}Case_Record_Type__c`]).toLowerCase() === ('Inbound_Call').toLowerCase()){
                        this.dispatchEvent(new CustomEvent('cancel'));
                      }
                      this.invokeWorkspaceAPI("closeTab", {
                        tabId: focusedTab.tabId
    ...
    if (result) {
      ...
      if (this.secondCaseId) {
        ...
        await updateRecord({ fields })
         ...
         // if(this.serviceType === 'Outbound_Communication'){
          let phoneField = this.template.querySelector(`lightning-input-field[data-field="${nameSpace}Phone__c"]`);
          let phonefieldValue;
          if(phoneField && phoneField.value){
            phonefieldValue = phoneField.value;
          } 
            generateOutboutCallEvent({
              caseId:this.secondCaseId,
              serviceId:this.secondService,
  ...
  asyncUpdatePrimaryFieldValues(event) {
    ...
    for (const key in this.orginalData) {
    this.orginalData[key].forEach((element) => {
      if (element.dependentField && ((!result[element.dependentField] &&  !this.skipFieldVisibility)
...
validateIncompleteReasons(event) {
  ...
  async handleAgeFields(isUnderEighteen) {
    try {
      this.showUI = false;
      for (const key in this.orginalData) {
        this.serviceFields.forEach((element) => {
          element.value.forEach((item) => {
        if (item.softRequiredBeloweighteen) {
          if(isUnderEighteen){
            item.softrequired = true;
          }else{
            item.softrequired = false;
          }
  ...
  let duplicateCheckFields = [];
    ...
    for(var fld in duplicateCheckFields){
      ...
      if(this.duplicatedataFldList[dupnamewithNS]){
        ...
        for(var refFld in duplicateCheckRefFields){
          ...
          if(dupCheckRefFldName == fieldName){
            ...
            dupRefErrMsg = 'Duplicate data found in '+dupRefErrLabel+'\n';
          }
        }
      }
      ...
      if(dupDataFld && dupDataFld.value ){
        ...
        if(!ErrorMessage){
          ErrorMessage = 'Duplicate data found in '+dupdatafieldLabel+'\n';
        }else{
          ErrorMessage = ErrorMessage+'Duplicate data found in '+dupdatafieldLabel+'\n';
        }
      }else{
